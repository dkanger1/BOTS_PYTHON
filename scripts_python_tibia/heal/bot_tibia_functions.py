...
window_name = 'Tibia - Mucas Marisco'
...
###############################################################
    ...
    if( battlepixels == 0 and hppixels > 50 and clock()%8 < 0.6):
        win32api.SendMessage(hwnd, win32con.WM_KEYDOWN, 0x73, 0)  ##F4
        win32api.SendMessage(hwnd, win32con.WM_KEYUP, 0x73, 0)
        cavebot(map_count)
    ...
    if(clock()%40 < 1):  
        win32api.SendMessage(hwnd, win32con.WM_KEYDOWN, 0x76, 0)
        win32api.SendMessage(hwnd, win32con.WM_KEYUP, 0x76, 0) 
    # if(clock()%20 < 0.4):  
    #     pyautogui.click(pyautogui.locateOnScreen('imgs/gold.jpg',region=(0,100,150,105), confidence = 0.6),button='right',clicks=3, interval=0.1)
